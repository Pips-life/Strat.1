...
def test_higher_timeframe_qof_context_confirms_execution_precision():
    ...
    assert result.execution_timeframe == "5m"
...
def test_price_only_context_cannot_satisfy_qof_alignment():
    ...
    result = planner.plan(
        side="SHORT", price=110.0, execution_zone=execution,
        execution_timeframe="5m", context_zones_by_timeframe=context,
        atr=2.0, stop=112.0, target=104.0, minimum_rr=1.35,
    )
    assert result.accepted is False
    assert result.aligned_context is False
...
def test_missing_context_does_not_create_direction_or_force_rejection():
    ...
    result = planner.plan(
        side="SHORT", price=110.0, execution_zone=execution,
        execution_timeframe="5m", context_zones_by_timeframe=None,
        atr=2.0, stop=112.0, target=104.0, minimum_rr=1.35,
    )
    assert result.accepted is True
    assert result.aligned_context is False
