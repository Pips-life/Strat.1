...
class ReplayRunner:
    """Run a Strategy through RiskEngine and the environment execution adapter.

    Realized P&L is fed back into risk checks during replay. Options history is
    intentionally left injectable through market_builder.
    """
    def __init__(self, bars: list[Bar], execution: ExecutionAdapter, strategy: Strategy,
        ...
        self.initial_equity = account_equity; self.equity = account_equity
        self.point_value = point_value; self.quantity_step = quantity_step
        self.events: list[ReplayEvent] = []; self.trades: list[ClosedReplayTrade] = []
        self._open: dict[str, tuple[Fill, float]] = {}; self._daily_pnl: dict[date, float] = {}
        self._daily_trades: dict[date, int] = {}; self.consecutive_losses = 0
        self.max_drawdown = 0.0; self.peak_equity = account_equity
        ...
        for bar in self.replay.stream():
            ...
            if signal.action in {"BUY", "SELL"} and signal.entry is not None and signal.stop_loss is not None and signal.take_profit is not None:
                ...
                if decision.approved:
                    ...
                    if result.fill:
                        self._daily_trades[day] = self._daily_trades.get(day, 0) + 1; self._record_fills([result.fill], bar, "ENTRY", signal.stop_loss)
    ...
    def summary(self) -> dict[str, Any]:
        wins = [t for t in self.trades if t.pnl > 0]; losses = [t for t in self.trades if t.pnl < 0]
        gp = sum(t.pnl for t in wins); gl = abs(sum(t.pnl for t in losses))
        return {"initial_equity": self.initial_equity, "final_equity": self.equity, "realized_pnl": self.equity-self.initial_equity, "trades": len(self.trades), "win_rate": len(wins)/len(self.trades) if self.trades else 0.0, "profit_factor": gp/gl if gl else (float("inf") if gp else 0.0), "total_r": sum(t.r_multiple for t in self.trades), "max_drawdown": self.max_drawdown}

    @staticmethod
    def _default_market_builder(bar: Bar) -> dict[str, Any]:
        return {"symbol": str(bar.metadata.get("symbol", "XAUUSD")), "price": bar.close, "atr": bar.atr or 0.0, "confluence": {}, "levels": {}, "price_action": {}, "liquidity": {}, "volatility": {}}
    @staticmethod
    ...
    def _record_fills(self, fills: list[Fill], bar: Bar, kind: str, stop_loss: float | None = None) -> None:
        for fill in fills:
            ...
            pnl = (fill.price-entry.price)*fill.quantity*direction*self.point_value - entry.commission-fill.commission
            risk_cash = abs(entry.price-stop)*entry.quantity*self.point_value
            r = pnl/risk_cash if risk_cash > 0 else 0.0
            self.trades.append(ClosedReplayTrade(fill.symbol, entry.side, entry.quantity, entry.price, fill.price, entry.timestamp, fill.timestamp, stop, pnl, r, kind))
            self.equity += pnl; day = fill.timestamp.date(); self._daily_pnl[day] = self._daily_pnl.get(day, 0.0)+pnl
            self.consecutive_losses = self.consecutive_losses+1 if pnl < 0 else 0
            self.peak_equity = max(self.peak_equity, self.equity); self.max_drawdown = max(self.max_drawdown, self.peak_equity-self.equity)
