...
class Strategy005(Strategy):
    ...
    def analyze(self, market: Any) -> dict[str, Any]:
        ...
        for s, names in (("BUY",(("s1","S1_REJECTION"),("s2","S2_REJECTION"))),
            ...
            for level,name in names:
                if self._rejection(candle,s,levels[level],tolerance):
                    ...
                    break
            if side!="WAIT": break
        if side=="BUY" and prior["close"]>=levels["pp"]: side,trigger="WAIT",None
        if side=="SELL" and prior["close"]<=levels["pp"]: side,trigger="WAIT",None
        stop=None
        ...
        rr=abs(levels["pp"]-price)/risk if risk>0 else 0
        return {"ready":True,"side":side,"price":price,"stop":stop,"target":levels["pp"],
    ...
    def generate_signal(self, analysis):
        ...
        return Signal(action=side,confidence=90,entry=float(analysis["price"]),
                      ...
                      metadata={**analysis,"strategy":self.id,"entry_model":"5M rejection at S1/S2 or R1/R2",
    ...
    def risk_parameters(self):
        return {"risk_cap":self.config.risk_cap,"risk_basis":"account balance",
                ...
                "entry_timeframe":"5m","entry_model":"5M rejection/reclaim at S1/S2 or R1/R2",
