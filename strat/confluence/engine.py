...
class ConfluenceConfig:
    weights: Dict[str, float] = field(default_factory=lambda: {"structure": 0.25, "options_flow": 0.20, "gamma": 0.15, "delta": 0.10, "iv": 0.10, "velocity": 0.10, "volume": 0.10})
...
class ConfluenceEngine:
    def __init__(self, config: ConfluenceConfig | None = None): self.config = config or ConfluenceConfig()
    @staticmethod
    ...
    def _score_direction(self, components: Mapping[str, float | None]) -> tuple[float, list[str]]:
        weighted=0.0; total_weight=0.0; missing=[]
        for name, weight in self.config.weights.items():
            value=components.get(name)
            if value is None: missing.append(name); continue
            weighted += self._normalize(value)*weight; total_weight += weight
    ...
    def _interactions(self, components: Mapping[str, float | None]) -> tuple[float,list[str],Dict[str,float]]:
        n={k:self._normalize(v) for k,v in components.items() if v is not None}; bonuses=[]; conflicts=[]
        def pair(a,b,label,threshold=70.0):
            if a not in n or b not in n: return
            if n[a]>=threshold and n[b]>=threshold: bonuses.append(min(n[a],n[b])/100.0*3.0)
            elif n[a]>=75.0 and n[b]<=30.0: conflicts.append(label)
        pair("structure","options_flow","structure_flow_conflict"); pair("structure","delta","structure_delta_conflict")
        pair("options_flow","delta","flow_delta_conflict"); pair("gamma","velocity","gamma_velocity_conflict")
        pair("iv","velocity","iv_velocity_conflict"); pair("volume","velocity","volume_velocity_conflict")
        bonus=min(self.config.interaction_bonus_cap,sum(bonuses))
        return bonus,conflicts,{"interaction_bonus":round(bonus,3),"interaction_pairs":float(len(bonuses))}
    def evaluate_directional(self,long_components,short_components,contradictions=(),derived=None):
        long_score,long_missing=self._score_direction(long_components); short_score,short_missing=self._score_direction(short_components)
        long_bonus,long_conflicts,long_interactions=self._interactions(long_components); short_bonus,short_conflicts,short_interactions=self._interactions(short_components)
        long_score=min(100.0,long_score+long_bonus); short_score=min(100.0,short_score+short_bonus)
        contradiction_list=list(contradictions); inferred=sorted(set(long_conflicts+short_conflicts)); total_penalty=self.config.contradiction_penalty*len(contradiction_list)+self.config.inferred_conflict_penalty*len(inferred)
        long_score=max(0.0,long_score-total_penalty); short_score=max(0.0,short_score-total_penalty)
