...
async function state(accountId: string) { return await getCache().get<{ running: boolean; strategy: string }>(cacheKey(accountId)); }
...
function normaliseSide(value: unknown): 'BUY' | 'SELL' | undefined {
...
function analyseTicks(ticks: Tick[]) {
  ...
  const needed = BASELINE_WINDOW + 2;
  if (samples.length < needed) return { expanding: false, reason: `waiting for velocity samples (${samples.length}/${needed})`, price: samples.at(-1)?.p ?? 0 };
  const recent = samples.slice(-(BASELINE_WINDOW + 2));
  const velocities: number[] = [];
  for (let i = 1; i < recent.length; i++) {
    const dt = recent[i].t - recent[i - 1].t;
    velocities.push(dt > 0 ? (recent[i].p - recent[i - 1].p) / dt : 0);
  }
  const current = velocities.at(-1) ?? 0;
  const previous = velocities.at(-2) ?? 0;
  const baselineValues = velocities.slice(0, -1).map(Math.abs).sort((a, b) => a - b);
  const middle = Math.floor(baselineValues.length / 2);
  const baseline = Math.max(baselineValues.length % 2 ? baselineValues[middle] : (baselineValues[middle - 1] + baselineValues[middle]) / 2, 1e-12);
  const expansionRatio = Math.abs(current) / baseline;
  const accelerationRatio = Math.abs(current) / Math.max(Math.abs(previous), 1e-12);
  const expanding = expansionRatio >= MIN_VELOCITY_RATIO && accelerationRatio >= MIN_ACCELERATION_RATIO;
  const direction = current > 0 ? 'BUY' : current < 0 ? 'SELL' : 'WAIT';
  const confidence = Math.min(99, 50 + 20 * Math.max(0, expansionRatio - 1) + 20 * Math.max(0, accelerationRatio - 1));
...
async function trade(api: string, accountId: string, body: Record<string, unknown>) {
  return await metaFetch(`${api}/users/current/accounts/${encodeURIComponent(accountId)}/trade`, { method: 'POST', body: JSON.stringify(body) });
}

async function execute002(accountId: string, api: string): Promise<RunnerState> {
  if (EXECUTION_MODE !== 'demo') {
    return { configured: true, state: 'READY', strategy: '002', activity: `Execution locked: PIPSLIFE_EXECUTION_MODE=${EXECUTION_MODE}; demo mode is required for this test build.` };
  }
  ...
  const ticks = (Array.isArray(ticksRaw) ? ticksRaw : []) as Tick[];
  ...
  const analysis = analyseTicks(ticks);
  ...
  const liveSide = normaliseSide(livePosition?.type);
  const pending = managedOrders.find(o => normaliseSide(o.type) === (liveSide === 'BUY' ? 'SELL' : liveSide === 'SELL' ? 'BUY' : undefined));

  if (livePosition && !pending) {
    const entry = Number(livePosition.openPrice ?? analysis.price);
    const opposite = liveSide === 'BUY' ? 'ORDER_TYPE_SELL_STOP' : 'ORDER_TYPE_BUY_STOP';
    const stopPrice = liveSide === 'BUY' ? entry - TRAIL_DISTANCE : entry + TRAIL_DISTANCE;
    await trade(api, accountId, { actionType: opposite, symbol: SYMBOL, volume: Number(livePosition.volume ?? 0.01), openPrice: Number(stopPrice.toFixed(2)), magic: MAGIC, clientId: CLIENT_ID, comment: 'PipsLife002' });
    return { configured: true, state: 'RUNNING', strategy: '002', activity: `Strategy 002 running: ${liveSide} ${livePosition.volume ?? 0.01} with ${TRAIL_PIPS}-pip opposite stop at ${stopPrice.toFixed(2)}`, positionCount: managedPositions.length, pendingCount: 1 };
  }

  if (!livePosition && analysis.expanding && (analysis.direction === 'BUY' || analysis.direction === 'SELL') && analysis.confidence >= MIN_CONFIDENCE && analysis.price > 0) {
    ...
    const stopPrice = side === 'BUY' ? entry - TRAIL_DISTANCE : entry + TRAIL_DISTANCE;
    await trade(api, accountId, { actionType: side === 'BUY' ? 'ORDER_TYPE_SELL_STOP' : 'ORDER_TYPE_BUY_STOP', symbol: SYMBOL, volume: 0.01, openPrice: Number(stopPrice.toFixed(2)), magic: MAGIC, clientId: CLIENT_ID, comment: 'PipsLife002' });
  ...
  const wait = analysis.expanding ? `velocity expanding ${analysis.direction} (${analysis.confidence?.toFixed(1)}%)` : analysis.reason ?? 'waiting for velocity expansion';
  return { configured: true, state: 'RUNNING', strategy: '002', activity: `Strategy 002 monitoring ${SYMBOL}: ${wait}`, positionCount: managedPositions.length, pendingCount: managedOrders.length };
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const saved = await state(accountId);
    ...
    if (saved.strategy === '002') {
      const result = await execute002(accountId, api);
    ...
    return NextResponse.json({ configured: true, state: 'ERROR', strategy: '002', error: e instanceof Error ? e.message : 'Bot execution failed' }, { status: 502, headers: { 'cache-control': 'no-store' } });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existing = await state(accountId);
    const strategy = requestedStrategy ?? existing?.strategy ?? '001';
    if (action === 'select') {
      await saveState(accountId, false, strategy);
      return NextResponse.json({ configured: true, state: 'SELECTED', strategy, activity: `Strategy ${strategy} selected in Vercel Bot Engine.` }, { headers: { 'cache-control': 'no-store' } });
    }
    if (action === 'stop') {
      await saveState(accountId, false, strategy);
      return NextResponse.json({ configured: true, state: 'STOPPED', strategy, activity: 'Trading stopped. Existing positions are left untouched.' }, { headers: { 'cache-control': 'no-store' } });
    }
    ...
    if (e instanceof Response) return e;
    return NextResponse.json({ configured: true, state: 'ERROR', strategy: requestedStrategy ?? '001', error: e instanceof Error ? e.message : 'Bot control failed' }, { status: 502, headers: { 'cache-control': 'no-store' } });
  }
}
