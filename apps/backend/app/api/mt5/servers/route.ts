...
export async function GET(request: Request) {
  ...
  if (!token) return NextResponse.json({ error: 'METAAPI_TOKEN is not configured' }, { status: 500 });
  ...
  try {
    const upstream = await fetch(`${PROVISIONING_URL}/known-mt-servers/5/search?query=${encodeURIComponent(query)}`, {
      headers: { Accept: 'application/json', 'auth-token': token },
      cache: 'no-store',
    });
    const body = await upstream.json().catch(() => ({}));
    if (!upstream.ok) return NextResponse.json({ error: 'MetaApi broker/server discovery failed', details: body }, { status: upstream.status });

    const brokers = Object.entries(body as Record<string, unknown>).flatMap(([broker, value]) => {
      if (!Array.isArray(value)) return [];
      return [{ broker, servers: value.filter((server): server is string => typeof server === 'string') }];
    });
