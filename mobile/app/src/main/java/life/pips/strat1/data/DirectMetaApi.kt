...
class DirectMetaApiClient(private val http: OkHttpClient = OkHttpClient()) {
    ...
    suspend fun refresh(token: String, account: MetaAccount, symbols: List<String>): Result<MetaSnapshot> = runCatching {
        ...
        MetaSnapshot(current, info.optDouble("balance", Double.NaN), info.optDouble("equity", Double.NaN), info.optDouble("freeMargin", Double.NaN), buildList {
            for (i in 0 until positions.length()) { val p = positions.optJSONObject(i) ?: continue; add(MetaPosition(p.optString("id").ifBlank { p.optString("positionId") }, p.optString("symbol"), p.optString("type"), p.optDouble("volume", 0.0), p.optDouble("openPrice", Double.NaN), p.optDouble("currentPrice", Double.NaN), p.optDouble("profit", Double.NaN), p.optDouble("stopLoss", Double.NaN), p.optDouble("takeProfit", Double.NaN))) }
        }, prices)
    }
    ...
    suspend fun candles(token: String, account: MetaAccount, symbol: String, timeframe: String = "1m", limit: Int = 20): Result<List<PriceCandle>> = runCatching {
        val url = "${clientBase(account.region)}/users/current/accounts/${account.id}/historical-market-data/symbols/${URLEncoder.encode(symbol, "UTF-8")}/timeframes/$timeframe/candles?limit=${limit.coerceIn(1, 100)}"
    ...
    private suspend fun request(method: String, url: String, body: String?, token: String, extra: Map<String, String> = emptyMap()): JSONObject = withContext(Dispatchers.IO) {
        val b = Request.Builder().url(url).addHeader("Accept", "application/json").addHeader("auth-token", token); extra.forEach { (k, v) -> b.addHeader(k, v) }
        if (body == null) b.method(method, null) else b.method(method, body.toRequestBody("application/json".toMediaType()))
        http.newCall(b.build()).execute().use { r -> val text = r.body?.string().orEmpty(); if (!r.isSuccessful && r.code != 204) throw IllegalStateException(error(text, "MetaApi request failed (${r.code})")); if (text.isBlank()) JSONObject() else JSONObject(text) }
    }

    private suspend fun requestArray(method: String, url: String, body: String?, token: String): JSONArray = withContext(Dispatchers.IO) {
        val b = Request.Builder().url(url).addHeader("Accept", "application/json").addHeader("auth-token", token); if (body == null) b.method(method, null) else b.method(method, body.toRequestBody("application/json".toMediaType()))
        http.newCall(b.build()).execute().use { r -> val text = r.body?.string().orEmpty(); if (!r.isSuccessful) throw IllegalStateException(error(text, "MetaApi request failed (${r.code})")); JSONArray(text) }
    }
