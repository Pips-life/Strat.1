...
private fun Strat1App(context: Context) {
    ...
    Scaffold(bottomBar = { NavigationBar { labels.forEachIndexed { index, label -> NavigationBarItem(selected = tab == index, onClick = { tab = index }, icon = { Text(if (index == 0) "⌂" else if (index == 1) "M" else "⚙") }, label = { Text(label) }) } } }) { padding -> when (tab) { 0 -> DashboardScreen(Modifier.padding(padding)); 1 -> Mt5Screen(context, Modifier.padding(padding)); else -> SettingsScreen(Modifier.padding(padding)) } }
}
...
private fun DashboardScreen(modifier: Modifier = Modifier) {
    Column(modifier.fillMaxSize().padding(20.dp)) {
        ...
        Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) { Text("Backend", style = MaterialTheme.typography.titleMedium); Text("MT5 connection is managed securely by the backend.") } }
    }
}
...
private fun Mt5Screen(context: Context, modifier: Modifier = Modifier) {
    ...
    LazyColumn(modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item { Text("MT5 Account", style = MaterialTheme.typography.headlineSmall); Text("Search the live MetaApi server directory, select the exact server, then connect.") }
        item { OutlinedTextField(broker, { broker = it }, Modifier.fillMaxWidth(), label = { Text("Broker name") }, singleLine = true) }
        item { Button(enabled = broker.trim().length >= 2 && !busy, onClick = { busy = true; status = "Searching MetaApi for broker servers…"; api.searchServers(broker) { result -> scope.launch { busy = false; result.onSuccess { matches -> serverList = matches; status = if (matches.isEmpty()) "No known servers found. Try the broker's exact name." else "Select the server matching your MT5 account." }.onFailure { status = "Server discovery failed: ${it.message ?: "network error"}" } } } }) { Text(if (busy) "Searching…" else "Find servers") } }
        items(serverList) { server -> Card(onClick = { selectedServer = server.serverName }, Modifier.fillMaxWidth()) { Column(Modifier.padding(14.dp)) { Text(server.brokerName, style = MaterialTheme.typography.labelMedium); Text(server.serverName, style = MaterialTheme.typography.titleMedium); if (server.serverName == selectedServer) Text("Selected", style = MaterialTheme.typography.labelMedium) } } }
...
private fun SettingsScreen(modifier: Modifier = Modifier) {
    Column(modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { Text("Settings", style = MaterialTheme.typography.headlineSmall); Text("Updates will be checked against the Strat.1 release service."); Text("Release channel: Stable"); Text("Current release: 0.1.0 (1)"); Text("Update policy: notify → download → Android install confirmation") }
}
