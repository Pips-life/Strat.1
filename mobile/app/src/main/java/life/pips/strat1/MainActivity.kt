...
private fun PipsLifeApp() {
    ...
    LaunchedEffect(Unit) {
        ...
        if (saved.accountId.isNotBlank() && saved.metaApiToken.isNotBlank()) account = meta.connectExisting(saved.metaApiToken, saved.accountId).getOrNull()
    }
    ...
    MaterialTheme(colorScheme = darkColorScheme(background = Bg, surface = Panel, primary = Cyan, secondary = Green, error = Red)) {
        Scaffold(containerColor = Bg, bottomBar = {
            NavigationBar(containerColor = Color(0xFF08111D)) {
                listOf(Tab.HOME to "HOME", Tab.METAAPI to "METAAPI", Tab.STRATEGY to "STRATEGY", Tab.WATCHLIST to "WATCH", Tab.ACTIVITY to "ACTIVITY").forEach { (t, label) -> NavigationBarItem(selected = tab == t, onClick = { tab = t }, icon = {}, label = { Text(label, fontSize = 8.sp) }) }
            }
        }) { pad ->
            when (tab) {
                Tab.HOME -> Home(Modifier.padding(pad), account, snapshot, flashData, selectedStrategy, running, armed, onOpenStrategy = { tab = Tab.STRATEGY }, onStartStop = { running = it; if (!it) armed = false })
                ...
                Tab.WATCHLIST -> WatchlistTab(Modifier.padding(pad), saved, snapshot, selectedSymbol) { value -> selectedSymbol = value.split(',').firstOrNull()?.trim().orEmpty().ifBlank { selectedSymbol }; saved = saved.copy(watchlist = value); scope.launch { context.saveConnection(value.let { saved }) } }
                Tab.ACTIVITY -> QuantitativeActivityPage(Modifier.padding(pad), status, account, selectedStrategy, running, armed, flashData, snapshot, selectedSymbol, engine)
            }
        }
    }
}
...
@Composable private fun Home(modifier: Modifier, account: MetaAccount?, snapshot: MetaSnapshot?, flash: FlashAlphaSnapshot?, strategy: TradingEngine.StrategyId, running: Boolean, armed: Boolean, onOpenStrategy: () -> Unit, onStartStop: (Boolean) -> Unit) {
    LazyColumn(modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
        ...
        item { CardBlock { Text("SELECTED STRATEGY", color = Muted, fontSize = 10.sp); Text(if (strategy == TradingEngine.StrategyId.STRATEGY_001) "001 — GEX / QOF DATA ZONES" else "002 — VELOCITY EXPANSION", color = Cyan, fontSize = 18.sp, fontWeight = FontWeight.Black); Text(if (strategy == TradingEngine.StrategyId.STRATEGY_001) "Live XAUUSD spot → FlashAlpha zones → strike confluence → dealer positioning → execution → management." else "Live tick-to-tick velocity drives direction and a 70-pip trailing stop.", color = Muted, fontSize = 11.sp); Button(onClick = onOpenStrategy, modifier = Modifier.fillMaxWidth()) { Text("OPEN STRATEGY SELECTOR") }; Button(onClick = { onStartStop(!running) }, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = if (running) Red else Green)) { Text(if (running) "STOP BOT" else "START BOT") }; Text(if (!running) "STOPPED" else if (armed) "LIVE TRADING ARMED" else "MONITORING", color = if (armed) Red else Cyan, fontWeight = FontWeight.Bold) } }
...
@Composable private fun MetaApiTab(modifier: Modifier, saved: SavedConnection, account: MetaAccount?, busy: Boolean, status: String, onSave: (SavedConnection) -> Unit, onConnect: (SavedConnection) -> Unit) {
    ...
    LazyColumn(modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
        item { Header("MetaApi Connection", "Direct account provisioning + direct trade API") }; item { Field("MetaApi auth token", token, { token = it }, true) }; item { Field("Existing MetaApi account ID (optional)", accountId, { accountId = it }) }; item { Field("MT5 login", login, { login = it }) }; item { Field("MT5 password", password, { password = it }, true) }; item { Field("MT5 server", server, { server = it }) }; item { Text("Credentials are encrypted locally with Android Keystore. No Pips-life trading backend is used.", color = Muted, fontSize = 10.sp) }
...
@Composable private fun StrategyTab(modifier: Modifier, saved: SavedConnection, account: MetaAccount?, snapshot: MetaSnapshot?, flash: FlashAlphaSnapshot?, tradeSymbol: String, flashSymbol: String, selected: TradingEngine.StrategyId, running: Boolean, armed: Boolean, status: String, onSelect: (TradingEngine.StrategyId) -> Unit, onFlashSymbol: (String) -> Unit, onSave: (SavedConnection) -> Unit, onRun: (Boolean) -> Unit, onArm: (Boolean) -> Unit) {
    var key by remember(saved.flashAlphaKey) { mutableStateOf(saved.flashAlphaKey) }; var symbol by remember(flashSymbol) { mutableStateOf(flashSymbol) }; var menu by remember { mutableStateOf(false) }; val price = snapshot?.prices?.get(tradeSymbol)?.let { (it.bid + it.ask) / 2.0 }; val p1 = remember(flash, price) { if (price != null) Strategy001Engine().evaluate(flash, price) else null }
    LazyColumn(modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
        item { Header("Strategy Control", "Shared execution engine • rules remain independent") }
        item { Box { Button(onClick = { menu = true }, modifier = Modifier.fillMaxWidth()) { Text(if (selected == TradingEngine.StrategyId.STRATEGY_001) "001 — GEX / QOF DATA ZONES" else "002 — VELOCITY EXPANSION") }; DropdownMenu(expanded = menu, onDismissRequest = { menu = false }) { DropdownMenuItem(text = { Text("001 — GEX / QOF DATA ZONES") }, onClick = { menu = false; onSelect(TradingEngine.StrategyId.STRATEGY_001) }); DropdownMenuItem(text = { Text("002 — VELOCITY EXPANSION") }, onClick = { menu = false; onSelect(TradingEngine.StrategyId.STRATEGY_002) }) } } }
        ...
        if (selected == TradingEngine.StrategyId.STRATEGY_001) {
            ...
            item { CardBlock { Text("GEX / GREEKS MARKET MAP", color = Muted, fontSize = 10.sp); if (p1 == null) Text("Waiting for live FlashAlpha + MetaApi price", color = Muted) else { Text("${p1.side?.name ?: "WAIT"} • confidence ${p1.confidence}%", color = if (p1.side == null) Muted else Green, fontSize = 20.sp, fontWeight = FontWeight.Black); Text(p1.reason, color = Muted, fontSize = 11.sp); p1.entryZone?.let { Text("ENTRY ${it.kind}: ${number(it.lower)} — ${number(it.upper)} (${it.source})", color = Cyan, fontSize = 11.sp) }; p1.exitZone?.let { Text("EXIT ${it.kind}: ${number(it.lower)} — ${number(it.upper)} (${it.source})", color = Purple, fontSize = 11.sp) }; p1.invalidation?.let { Text("INVALIDATION: ${number(it)}", color = Red, fontSize = 11.sp) } } } }
