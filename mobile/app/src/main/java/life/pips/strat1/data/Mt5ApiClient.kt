...
class Mt5ApiClient(private val baseUrl: String = BuildConfig.BACKEND_BASE_URL.trimEnd('/'), private val http: OkHttpClient = OkHttpClient()) {
    suspend fun findServers(query: String): List<Mt5Server> = withContext(Dispatchers.IO) {
        ...
        http.newCall(request).execute().use { response -> val body = response.body?.string().orEmpty(); if (!response.isSuccessful) throw IllegalStateException(error(body, "Server discovery failed")); val items = JSONObject(body).optJSONArray("brokers") ?: JSONArray(); buildList { for (i in 0 until items.length()) { val x = items.getJSONObject(i); add(Mt5Server(x.optString("id"), x.optString("brokerName"), x.optString("serverName"), x.optString("environment", "real"))) } } }
    }
    ...
    suspend fun connect(login: String, password: String, server: String, broker: String = ""): Mt5ConnectionResult = withContext(Dispatchers.IO) {
        ...
        http.newCall(request).execute().use { response -> val body = response.body?.string().orEmpty(); if (!response.isSuccessful) throw IllegalStateException(error(body, "MT5 connection failed")); val x = JSONObject(body); val result = Mt5ConnectionResult(x.optString("accountId"), x.optString("state", "UNKNOWN"), x.optString("connectionStatus", "CONNECTING"), x.optString("server", server), x.optString("login", login), x.optString("sessionToken")); SessionStore.token = result.sessionToken; result }
    }
