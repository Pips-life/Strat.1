                    ...
                    try {
                        ...
                        callback(Result.success(buildList {
                            for (i in 0 until brokers.length()) {
                                val b = brokers.getJSONObject(i)
                                val broker = b.optString("broker")
                ...
                response.use {
                    ...
                    try {
                        ...
                        if (!it.isSuccessful) {
                            ...
                            val message = details?.optString("message")?.takeIf { it.isNotBlank() } ?: json.optString("error").ifBlank { "MT5 connection failed" }
                            callback(Result.failure(IOException(message))); return
                        }
                        callback(Result.success(Mt5ConnectionResult(json.optString("accountId").ifBlank { null }, json.optString("state", "UNKNOWN"), json.optString("server", server))))
